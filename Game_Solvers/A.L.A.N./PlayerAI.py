...
class PlayerAI(BaseAI):
    ...
    def getMove(self, grid: Grid) -> tuple:
        ...
        child, _, mvs = self.maximizeMove(grid, float('-inf'), float('inf'), start_time)
    ...
    def moveUtilityFn(self, _grid):
        ...
        # return -abs(manhattan_distance([x,y], [x1,y1]) - 2)
        x1, y1 = self.getOpponentPos(grid)
    ...
    def isFinalState(self, _grid, cur_time):
        grid = _grid.map
        x,y = self.getOpponentPos(grid)
    ...
    def isFinalMoveState(self, _grid, current_time):
        # grid = _grid.map
        # x,y = self.getMyPos(grid)
        # for i in range(max(0, x-1), min(len(grid), x+2)):
        #     for j in range(max(0, y-1), min(len(grid[i]), y+2)):
        ...
        if len(neighbors) == 0 or now - current_time > 3.5:
            return True
        return False

    def maximizeMove(self, grid, alpha, beta, cur_time, depth=0):
        if depth == MAX_DEPTH:
            return None, self.moveUtilityFn(grid), None
        if self.isFinalMoveState(grid, cur_time):
            # print('depth:', depth)
            return (None, self.moveUtilityFn(grid), None)
        ...
        for child in mvs:
            _, utility = self.minimizeThrow(child, alpha, beta, cur_time, depth+1)
    ...
    def minimizeThrow(self, grid, alpha, beta, cur_time, depth=1):
        if(depth == MAX_DEPTH):
        ...
        for child, prob in zip(thrs, probs):
            _, utility, _ = self.maximizeMove(child, alpha, beta, cur_time, depth+1)
    ...
    def minimizeMove(self, grid, alpha, beta, cur_time, depth=1):
        # spaces = "\t"*depth
        # print(f'{spaces} min')
        if depth == MAX_DEPTH:
            return None, self.utilityFn(grid)
        if self.isFinalState(grid, cur_time):
            return None, self.utilityFn(grid)
        min_child, min_utility = None, float('inf')
        for child in self.getChildrenMove(grid, self.opp_num):
            _, utility, _ = self.maximizeThrow(child, alpha, beta, cur_time, depth+1)
            if(utility < min_utility):
                min_utility, min_child = utility, child
            if(min_utility <= alpha):
                break
    ...
    def maximizeThrow(self, grid, alpha, beta, cur_time, depth=0):
        # spaces = "\t"*depth
        # print(f'{spaces} max')
        if(depth == MAX_DEPTH):
        ...
        for child, prob in zip(thrs, probs):
            _, utility = self.minimizeMove(child, alpha, beta, cur_time, depth+1)
    ...
    def getTrap(self, grid : Grid) -> tuple:
        ...
        """
        start_time = time.process_time()
        child, _, thrs = self.maximizeThrow(grid, float('-inf'), float('inf'), start_time)
        # print('throw options:')
